...
| Melee weapon purchase | Bought from the gladiator panel's PRIMARY slot ✅ | **The same shape: a rack on the man's own page** (2026-09-17) | Domina arms a gladiator where it shows him; we do the same, on the roster's detail column rather than at the armoury counter — arming a man is a decision about the man. The price is the weapon's output rate (damage ÷ attack cycle), with a share for the grip that catches and the dose. **What we learned by building it:** arming the two melee classes with their own implements is a loss at any price, so the class layer's weakness was never the missing counter (GDD Open Decisions #21 closed, #19 reopened). |
