...
| **22** | **The pull-out order in a played season** | Opened 2026-09-17 by six hand-played seasons: five of the six players reported `pull:` as broken, and they were reading it correctly. The order is the core's `RetreatWhenLosing`, which fires only when the party is **both** under its health share **and** outnumbered on the field (`Combat/IRetreatPolicy.cs`), so a party of four against one or two enemies cannot trip it until three of them are already down, and a lone man never trips it at all. As a **batch policy** that is a sensible class and nothing is wrong with it; as **the player's hand on the key** (§5, the fight is watched and the pull-out is a key) it is not the same thing. Three ways out: (a) `--play` binds `pull:` to a plain health threshold, leaving the batch policy alone; (b) the outnumbered clause stays and the screen states it, which is what the harness does today; (c) the game's own key is defined first and the harness copies it. **Not decided — the user's call**, and every played season before it is read with this beside it |
