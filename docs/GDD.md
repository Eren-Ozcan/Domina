...
| **Will** | `Willpower` (50) | Seppuku resistance, the panic check, and the brake on morale's falls. **In the core since 2026-09-10**; trained by the fifth drill, **Meditation**, which has no secondary stat |
...
- **Morale** — the warrior's **condition**. Low morale breaks stats; it is linked to Will
  ...
  defeat, a fallen comrade, going hungry. **Numbers locked 2026-09-10** (build-order step 6):

| Number | Value | How it was settled |
|---|---|---|
| The stat multiplier | **×0.94 at morale 0, ×1.03 at 100**, ×1.00 at 50 | `3v3`, 20.000 fights: victory 63.57% / 69.64% / 75.24%. The first band tried (0.90/1.05) swung 17.6 points and a 0.70 floor collapsed the side (31.09%) — the multiplier lands on six stats at once |
| A quiet day | **settles toward 50, never above** (+2), plus a **drift** of 1/day from either side | Measured: a flat daily gain pinned every roster at 100 within a month and made the lower half of the band unreachable |
| Victory / defeat | **+8 / −10** | Above the middle is bought, never drifted into |
| Panic on top of a defeat | **−6** to the man who ran | Panic has to cost, or breaking is free |
| A comrade's death | **−6 per man lost** | It weighs on everyone who came back |
...
scaling them would take back wounds the warrior already had.
...
| **Whiffing** | If the target dies, leaves the field or **starts to flee**, the charge is wasted: the warrior drops to `Idle` and the reward is not taken. The charge keeps its own time limit — a charge that does not arrive within it is wasted the same way. (This is not the **fight's** time limit: `BattleOutcome.TimeLimit` was removed in the decision round and taken out of the code on 2026-09-10, so a fight lasts until someone falls. What is left is `CombatTuning.StallGuardSeconds` — a safety ceiling far above any real fight, and reaching it is an anomaly, not a draw) |
...
#### The kinds' appetites (Open Decision #3, closed 2026-09-13)

The weights above are what **everyone** runs; what separates a cutthroat from a duelist is only how
much each term is worth to him. `Combat/TargetProfile.cs` is five **multipliers** over the five weights
— never absolute numbers, so a round that retunes what a bare region is worth moves every kind with it
and a profile stays a statement about character. `TargetProfile.Default` is all ones, which is the
behaviour every figure measured before profiles existed was measured on.

| Kind | Profile | What it is |
|---|---|---|
| Collector | crowd ×0.5, wound ×1.2 | He collects in numbers — two of them stand on one man rather than take one each |
| Cutthroat | wound ×1.8, bare region ×1.5, stickiness ×0.6 | He finishes, he does not duel: the knife goes where the blood already is |
| Duelist | wound ×0.4, crowd ×1.8, stickiness ×1.6 | He wants his own opponent and will not leave him — the trial is the point, not the kill |
| Kabukimono | distance ×2.0, wound ×0.6, stickiness ×1.3 | At Speed 28 under a tetsubo, walking the arena is how a fight is spent without landing a blow |
| Senior student | distance ×0.8, bare region ×1.4, crowd ×0.8 | The yari's reach makes the road cheap, so he is taught to take the opening |
| Kurogane | wound ×1.5, bare region ×1.6, stickiness ×0.8 | The head of the school takes whatever is open |

**The dojo's own men carry no profile**, and that is a design line rather than an omission: the player
...
**+0.00 ±0.06**. A season cannot feel it, which is exactly what a character trait should cost — the
kinds are already separated by their stats and weapons, and a manner that also moved survival would be
a difficulty change wearing a personality. The fights themselves do come out differently (a test sweeps
twelve seeds to prove the appetite reaches the field), and the lever is now in place should a kind ever
need to become a threat rather than a manner.
...
**The class list (locked 2026-09-10, in the core since build-order step 4):**

| Class | Opens | The implement that pays it | Closed by |
|---|---|---|---|
...
| Case | Multiplier | Measured (20.000 fights, `losing:0.7`) |
|---|---|---|
| Right class, right implement | 1.0 × the implement | jitte 78.65%, sai 80.47% |
| Right class, wrong implement or empty-handed | `UnskilledCatchImplementFactor` **0.10** | torite + katana 81.07% |
| **No class**, right implement | **0** | jitte 41.76% — the same weapon, no class |
| No class, poison | `UnclassedPoisonFactor` **0.6** | 65.12% against the trained 76.09% |
| No class, range | `UnclassedRangeFactor` **0.85** | 82.37% against the trained 83.52% |
...
> 78.65%, in front of ō-yoroi 63.63% against 35.59%. A catch is worth a lot per event, so a rare one
> collects most of the value while the jitte pays 8 damage a strike for the frequency. This is Open
> Decision **#19** widening, and the answer belongs to the implements' prices, not to the class
> layer. Phase 9.
...
**In the dojo since 2026-09-10 — `Dojo/Tribunal.cs`.** The rules above are the **live** half: they run
on the wall clock and were written for a stream. Until this date nothing outside `Honor/` called them,
so honour was a number the game wrote and never read — a broken promise, a rout and a missed week all
cost honour and cost nothing. The dojo's half restates the same rule on the **day**:

- a warrior below the threshold is **summoned when the day closes**, and answered when the **next** day
  closes. The day in between is the crowd's; on a silent dojo it is what keeps the verdict from being
  an execution the player never saw coming
- **one man stands at a time**, exactly as in the live queue
- a pardon pulls him to 45 and buys **14 days** of immunity — the live 15 minutes is nothing to a
  calendar, and a pardon that expired overnight would be worthless
- the sword goes through the roster like any other death: permadeath, the name back into the pool, the
  man counted on the closing screen
- the verdict's die is a function of the **seed and the day**, and the queue and the pardons are in the
  save: a reload is not a reroll and not a way out
- what does **not** go into the save is the voices already cast — they belong to a live chat that is
  not there when the file is opened again

When the chat layer arrives it casts votes into this same summons. There is one rulebook.
...
The two implements (22 damage / jitte 1.10 s, sai 1.15 s since 2026-09-19; 15 / 1.00 / 1.05 before it)
are deliberately in the **blunt**
...
- **One 7-day tick, two consequences (2026-09-10).** The compulsory-fight rule **stands** and the
  ...
  - **In the core since build-order step 7 (2026-09-10), and the honour number is locked.** The rule
    as measured is not "a quiet week costs honour" but **"a week you chose to sit out costs honour"**.
    What the penalty finally reaches is the **tribunal** (§6), which was written the same day: until
    then nothing in the game read the honour number at all, so this penalty — locked at 5 precisely
    because honour pushes toward the seppuku threshold — was arithmetic with no consequence:

    | Number | Value | How it was settled |
    |---|---|---|
    | Missed-week honour penalty | **5** per living warrior | 200 hiding dojos against 200 ordinary ones, 180 days each. Honour's own decay (0.5/day = 3.5 a week) sets a hard floor: at **4 or below the rule is inert** — the hiding dojo settles at honour 41 and nobody ever reaches the seppuku threshold. At 5 the hider ends at 22 and 99% cross by day 91, while the ordinary dojo ends at 47 and its crossing rate goes 8.5% → 21%. At 6 it is 41.5% of **ordinary** dojos below the threshold, at 8 it is 59% |
    | Grace weeks | **1** | The first quiet week of a run is free. It was written as the item that would separate hiding from bad luck and **the measurement said otherwise** — the ordinary dojo's longest run of quiet weeks is 12.9, one block at the end of the season, the same shape a hider's is. It stays as a small mercy, not as the rule that closes the exploit |
    | Days with somebody fit | **4** of the 7 | This is what actually separates them. Without it the ordinary dojo pays for 12.3 of its 13.4 quiet weeks, because its quiet weeks **are** the weeks its party is in the infirmary; with it, 5.1. A roster lying in bed is not choosing anything. Swept 1-7 the number itself barely moves (paid weeks 5.5 → 4.9): a decision knob, not a survival one |

    ⚠️ **Still unmeasured:** whether the map pressure adds anything on top of the honour penalty —
    the settlement map is not in the core yet, and what step 7 put on screen is only the shared
    counter (`Next move: n days`)
...
**In the core since build-order step 7 (2026-09-10).** `Campaign/Season.cs` holds the clock and the
books, `Campaign/FinalNight.cs` the five bouts. What the code fixes about the design:

- **The gate is read on the last day and nowhere else.** Three heads brought in on day 12 are worth
  exactly what three brought in on day 179 are — the gate asks whether the season did the work, not when
- **A bout is not a day.** No day closes between the bouts: no infirmary day burns, no upkeep is paid,
  no reward is collected. Gold on the last night would be gold with nothing left to buy; the night's
  only currency is the men still standing
- **A won bout that leaves nobody able to stand is a night lost**, decided on the spot rather than
  reported one bout later
- **A roster that runs out closes the dojo the same day**, whatever day it is: with nobody to send,
  the remaining countdown is one the player cannot act on
- **The five bouts** come at powers **2.2 / 2.4 / 2.6 / 2.8 / 3.2** against **2 / 3 / 3 / 4 / 1** men.
  The fifth is `Adversaries.KuroganeHead`, deliberately kept **out of** the encounter pool: he is met
  once, at the end, or not at all
- **A wound is carried onto the field as health, not as a refusal (2026-09-10).** "No healing between
  the bouts" had to be given a body. The dojo records a wound in **days**, and the first version of the
  night simply refused anybody carrying one — measured, that made the night unplayable: three quarters
  of the dojos that reached it could field **nobody**, and dropping the bouts below day 1's power did
  not move it. So a man who can stand, stands, with what is left of him: **5% of his health per
  infirmary day**, and past **10 days** he cannot be carried to the field at all
- **There is no withdrawing from a bout.** Pulling out is losing it, and losing it ends the run

**Measurement of the last night (2026-09-10, 200 dojos × 180 days per row, the party rested for the
last week, no retreat on the night).** It took two passes, and **the first pass was measuring the
wrong dojo**: against a policy that never built a facility, never filled a post and never chose a
path, the night was unwinnable, and the bouts were softened to 1.8-2.8 to compensate. Re-measured
against a dojo that plays the game, that softened night was won by 78% of well-stocked dojos — the
ending had become a formality. The powers went back to the design's own line (at and above the
curve's ceiling of 2.2) and the calibration moved onto the **crowd**, which is the axis that actually
carries it: between powers 2.2-3.2 and 2.6-3.6 the night moves three points, between two men in a
bout and four it moves forty. Past a point power only adds health and strength — an enemy's accuracy,
defence and evasion are capped at 95 — and a developed party answers that with its kit.

| The dojo that walks into the night | Won all five |
|---|---|
| built its school, filled its posts, chose its paths — **eight** men | **38.5%** |
...
(3/4/4/5) shuts it outright at 0.5%, which is a wall rather than an ending. ⚠️ The sim's night policy
is still crude (the strongest men standing, bout after bout; it does not hold a fresh man back for
Kurogane), so a player should beat these figures
...
- **The roster's ceiling is a building (2026-09-10).** The dojo opens with room for **six** — the
  ceiling docs/STORY.md gives the dead master's school, and where the roster **starts** rather than
  where it ends. The **quarters** branch raises it: barracks 300 gold / 8 days and long house
  600 / 12, **+2 beds each**. No post (a roof needs nobody) and no half-efficiency (half a bed houses
  nobody). Hiring is refused when the beds are full; the dead and the released free theirs.

  **Measured** (300 dojos × 180 days, Master, full school): a dojo with **eight** beds from day one
  wins the last night **29.7%** of the time and one with ten **35.0%**, while a dojo that opens at six
  and buys its way to nine wins **10.7%** — and one that builds quarters and nothing else wins
  **0%**. Two things fall out of that. Beds alone are not a strategy: men with no school behind them
  never pass the fourth bout. And **a bed bought in the fifth month holds a raw recruit** — instant
  construction moved the night by three tenths of a point, so it is not the calendar, it is that the
  men who win the last night are the men who had a season to grow. Depth bought late converts gold
  into bodies rather than into wins, which makes the branch an **early** commitment competing with the
  school tiers for the same purse — the honest shape of the system, not a number to be tuned away
...
**In the core since build-order step 5 (2026-09-10).** The school tree **is** the facility tree —
the tiers stayed as upgrades and every node became a building:

| Number | Value | How it was settled |
|---|---|---|
| Build time | 6 / 10 / 14 days by tier, 12 for a class hall, 5-8 for a situational building | Swept ×0-×2: it moves **occupancy** (51 post-days against 26), not survival |
| Branch post wage | **6 gold a day** (situational **4**) | Swept 0-16 over 1500 dojos: post-days 44 → 19, dojos closed 64.3% → 65.9%. A decision knob, not a survival knob |
| Empty building | **half** its own bonus (×1.30 becomes ×1.15) | Swept 0-1: survival flat; the rule stands on the design promise, not on a measured edge |
...
>
> **The rack does not go through this branch (2026-09-17).** Buying a man a *different* weapon is
> ungated — no building, no smith, no facility — and it is done on his own page rather than at the
> armoury (§11, Open Decision #21). The forge and the rack answer different questions: the forge makes
> the weapon in the hand better and needs the branch's third tier to do it, while the rack decides
> **which** weapon is in the hand and is the counter the class layer was missing. Both cost the same
> thing beside the gold — a new name, so the mastery stays with the weapon it was learned on.
...
long breath, quiet mind, swift foot), each adding **points to one stat** — points, never a multiplier,
because morale and mastery are already multipliers and a third one would compound past measuring.
**The prices are per charm and were measured (2026-09-13): iron gate 120, swift foot 80, steady hand
40, long breath 40, quiet mind 30.** **The size of the blessing is per charm** and was measured
(2026-09-12): iron gate **+4** defence, swift foot **+8** evasion, steady hand **+12** accuracy,
quiet mind **+6** will, long breath **+15** stamina. The points carry the difference where the stat can
convert one; where it cannot, the **price** carries it — a fair exchange rate rather than one shelf
price, because with one price the strongest charm is the right answer for every man and the shelf
stops being a choice. They are transferable and sellable back at **half**. The slots are
...
| Number | Value | How it was settled |
|---|---|---|
| The blessing, per charm | **iron gate 4 · swift foot 8 · steady hand 12 · quiet mind 6 · long breath 15** | One size for all five was measured on 2026-09-12 (4000 dojos × 180 days, every system on, each run with every man wearing the same charm) and the five stats turned out not to be worth the same point for point. At +6 each and **at zero price**, last nights won: iron gate **32.8%**, swift foot 19.3%, steady hand 14.1%, quiet mind 9.9%, long breath 9.6% — against a charmless dojo's 9.8%. So the equal prices were a lie the points were telling. Swept per charm at the full 120 gold, iron gate **+4** and swift foot **+8** land level with each other (nights won 15.5% and 15.2%, dojos closed 24.1% and 23.4%, against the charmless 9.8% and 28.0%) and that is the rung the charm is meant to sit on: clearly worth buying, never a landslide. The prices were then measured charm by charm on 2026-09-13 — the row below |
| ⚠️ Three charms that points cannot fix | **steady hand, quiet mind, long breath** | They saturate below the rung: accuracy goes 8.8 → 10.2 → 9.9 → 9.8% of nights won at +6 / +12 / +18 / +30 (the hit chance runs out of room), while will and stamina sit **on or below** the charmless dojo's figures at any size (+20 will 7.4%, +40 will 7.5%, +40 stamina 6.7%) — Will is **clamped to 0-100** wherever it is read and training already carries a man to 90, so most of a large blessing is thrown away, and a fight ends long before a bigger stamina pool binds. Will and stamina are therefore **left at their small sizes (+6, +15)** rather than inflated along a flat curve: a +40 on the temple's stall would read as the best buy on the shelf while doing nothing. The honest reading is that **a stat point is the wrong currency for these three**: what they need is a job that is not a stat — the drill's mastery rate, the panic check inside the dojo, the recovery between fights. That is a design decision and it is **open**, not guessed at. **Re-measured 2026-09-12 (tenth round), on the written-down bed of `docs/PROGRESS.md` (1600 dojos × 180 days, `--accept-ratio 2.0`, every system on): at the sizes above and the full 120 gold, against a charmless dojo's 9.4% of nights and net 24.4 — iron gate **14.7% / 22.9**, swift foot **13.0% / 23.0**, steady hand 8.8% / 21.4, quiet mind **5.9% / 18.4**, long breath **5.6% / 18.2**. Standard error is about ±0.75 points, so steady hand is noise on a 120-gold bill and the two dead charms are **worse than wearing nothing**: the gold leaves and nothing comes back. **Then the jobs were given (tenth round, the same day): will buys the drill's rate (§11) and the stamina pool was made to bind (§5).** Re-measured on the re-locked bed (800 dojos, charmless 10.1% of nights / 30.4% closed / net 30.1): quiet mind **9.8% / 29.0% / 23.0**, long breath **9.0% / 27.3% / 23.1**. Both have stopped being **worse than nothing** — they now buy survival (fewer dojos closed) rather than nights — but neither clears its 120 gold: the charm-buying run also trains less, because the gold spent on the stall is gold the school does not get. **Closed 2026-09-13: the price carries the rest.** All five were then priced one charm at a time, and the single-seed readings quoted earlier in this row were re-taken over six seeds — a one-seed campaign reading of the last night carries about ±1.3 points of its own, so the "±0.75" above was optimistic. The prices and the paired figures are in the row below. |
| The stall, closed 2026-09-13 | **iron gate 120 · swift foot 80 · steady hand 40 · long breath 40 · quiet mind 30** | Measured one charm at a time, **six seeds × 1600 dojos × 180 days**, each seed against its **own** charmless control. The protocol is part of the finding: a single seed's reading of the last night swings about **±1.3 points** run to run — wider than the binomial error earlier rounds assumed — so a one-seed sweep cannot tell a 2-point effect from nothing, and every figure here is a paired mean over six seeds. **The rung is read in nights won**, against a charmless dojo's **13.7%**: a charm turns gold into victories and the net it costs per fight is the price of that, not a fault. At the locked prices the gains are iron gate **+3.4**, swift foot **+3.8**, steady hand **+3.7**, long breath **+3.7**, quiet mind **+2.8** — one rung, five charms, no landslide. **One price for all five fails in both directions**: at 120 gold steady hand is *worse than wearing nothing* (**−2.1** nights, −5.8 net) and quiet mind (+0.6) and long breath (+1.1) are noise, while at 40 gold iron gate is a landslide (**+10.5** nights, 13.7% → 24.2%, dojos closed −4.9) and the shelf collapses into a single right answer. The sweep behind each price: iron gate 40 / 60 / 120 = +10.5 / +7.9 / +3.4, swift foot 40 / 80 / 100 / 120 = +7.6 / +3.8 / +2.6 / +2.1, steady hand 40 / 60 / 120 = +3.7 / +1.2 / −2.1, quiet mind 20 / 30 / 40 / 120 = +3.1 / +2.8 / +2.2 / +0.6, long breath 40 / 120 = +3.7 / +1.1. Quiet mind at 20 and at 30 are the same reading inside the noise, so the **dearer** of the two was taken |
...
**The rack (2026-09-17, a price that is a proposal).** A melee weapon can now be bought for a man who
is already on the roster, and it is bought **on his own page** — the roster's detail column, under his
path and beside his charms — not at the armoury counter. Until this day nothing in the dojo sold one:
armour, the throwing slot and repairs were for sale and the forge only reworked the blade already in
the hand, so a torite trained in the dojo's own hall could never be handed a jitte (Open Decision #21).

The rack prices a weapon by its **output rate** — damage divided by the attack cycle — because damage
alone would sell the nodachi (34 a swing, 1.60 s) far above the katana (22, 1.10 s) when the two put
out nearly the same work per second. What the rate cannot see is paid for as a share: the grip that
catches (`WeaponCatchPremium` 0.5, scaled by the implement's catching skill) and the dose on the blade
(`WeaponPoisonPremium` 1.5).

| Weapon | Rate | Rack price |
|---|---|---|
| Tantō | 15.3 | 123 |
| Yari | 18.5 | 149 |
| Tetsubo | 19.4 | 155 |
| Katana | 20.0 | 160 |
| Poisoned tantō | 8.2 | 165 |
| Nodachi | 21.2 | 170 |
| Jitte | 20.0 | 240 |
| Sai | 19.1 | 249 |

At `WeaponGoldPerDamageRate` **8** a change of weapon stands beside a new man (150) and well under
reforging the blade already in the hand (a katana, 264), which is the intended ordering — the rack must
not be the cheap way past the forge. **The size is not locked and was not swept**, because the first
sweep on it found that arming a class with its own implement is a loss at any price (#21, #19): a price
cannot be measured for a purchase nobody should make yet. Two rules come with it, both the counter's
own: nothing is bought back, and the **mastery does not travel** — it is kept per weapon name, so a
veteran who changes weapon gives up what he built and gets it back untouched if he ever buys that
weapon again.
...
| ~~**14**~~ | ~~**Scarcity and a fourth resource**~~ | **Closed 2026-09-10 with build-order step 6.** Sake **is** the fourth stock, and it is the only one the day's bill never buys: it sits in the store until the player calls a **feast** (one measure per living warrior, +15 morale to the roster, then a 7-day wait). Scarcity works through morale rather than through a second hunger counter — a hungry day costs morale, and a dojo that cannot feast cannot buy its way out of a bad week. A daily sake drain was rejected: it would only have been a second food |
| ~~**15**~~ | ~~**A save backup must not be an undo gate**~~ | **Closed 2026-09-12.** The backup is rolled on every write (`dojo.json.bak`) and read in exactly one case: the current save **cannot be read at all**. It is never offered as a choice, there is no "go back to the previous day" anywhere in the game, and a save that loads with warnings is a save that loaded - merge-on-load has already done its work on it. When the fallback fires it **says so**, the same honesty merge-on-load owes. Deleting the save deletes the backup with it, so a new season cannot fall back into a dead one. A player who insists can copy a file on disk; that line is deliberate and it is outside the game. The fallback lives in the core (`DojoSaveFile.LoadWithBackup`) so it is tested, while the file handling stays in Godot |
...
| ~~**19**~~ | ~~**The catching implements are priced wrong**~~ | **Closed 2026-09-12 — one point of damage.** The jitte and the sai went from **14 to 15**, and both halves of the item closed with it: in a duel the catching warrior is now level whichever he carries (jitte **77.07%** against katana **76.13%**, 40.000 fights a row), and against a two-handed enemy the **brake is back** — jitte **33.39%** against the same warrior's katana at **32.46%** and the classless control's **30.37%**. The armoured enemy stays the katana's (38.31% against 56.24%), which is where §7 already put it, and the hard zero holds: a classless jitte is still 42.20%. The curve is steep enough that the number had to be found rather than reasoned — at 14 the implement loses the duel by 4.7 points, at 16 it wins it by 5.7. **The original finding:**  Measured 2026-09-10, and **not** caused by the stun-drop rule: with that rule switched off and 100.000 fights (the 20.000-fight bed cannot resolve a margin this thin — its standard error is ±0.34 points), `jitte-heavy` takes **34.65%** against `katana-heavy`'s **34.38%**. The catch round had locked the opposite as a brake — "the catching implement is still the wrong choice against an enemy carrying a nodachi" — and block later thinned that margin to 0.24 points; it has now crossed zero. `CatchTwoHandedFactor` (0.75) therefore no longer holds anything back. Nothing was re-locked on the strength of one measurement: this is a Phase 9 item, and the lesson is that a margin under ~0.5 points cannot be defended on a 20.000-fight bed. **Widened 2026-09-10 (build-order step 4):** with the class layer in, the comparison can be made between two warriors of the **same** class, and the catching implement then loses everywhere — the torite wins 81.07% with a katana against 78.65% with a jitte in a duel, 63.63% against 35.59% in front of ō-yoroi, 36.65% against 34.67% versus a two-handed enemy. The cause is that a catch pays **per event**, so the weak 0.10 floor collects most of its value while the jitte pays 8 damage a strike for the frequency. The item is now "the catching implements are priced wrong", and it is answered with their damage/speed/disarm share, not with the class numbers  **Reopened 2026-09-17 by the rack's first campaign reading.** The gold price was closed with one point of damage on the battle bed; the season disagrees. With a counter that can finally arm a trained torite (#21), arming him is a **loss** — his last night falls 19.1% to 1.8% and the dojo's net per fight 35.7 to 20.6, and the same collapse happens when the weapon is given away for 9 gold, so it is the implement and not its price. The item is therefore not the shelf price but the **combat** numbers: the jitte's and the sai's damage, speed and disarm share, and the dose that stands beside them. Phase 9. **Measured 2026-09-19, and the first finding is that the 2026-09-12 record above is stale by 21 points:** on today's build the same duel reads katana **72.14%**, jitte **55.17%** (40.000 fights). The knockout battery names the cause — the **catch's own stamina price**. With `--catch-stamina 0` the jitte beats the katana 77.72% to 72.02%, and with stamina never scarce (`--stamina-regen 100`) it beats it 79.89% to 73.39%; at the locked 16 it reads 55.13%. Cheaper *attacks* make it worse (attack stamina 8 → jitte 44.18%), because that saving is spent by the enemy too. **The proposal, not locked:** implement damage **15 → 18** (duel parity at 73.65%, and ahead of the katana against a two-handed enemy, 28.19% to 23.26%) and `CatchStaminaCost` **16 → 8**. The season still refuses it — 800 dojos × 180 days reads 10.5-12.2% of last nights won against a control's 19.1-20.2% — because the night is five sudden-death bouts against armoured enemies and armour subtracts a **flat** amount (`Battle.cs:1653`), which punishes a low-damage implement superlinearly; only damage 22, the katana's own, closes the season (20.4%), and that ends the trade. The gap left is therefore a **policy** question, not a number: `--arm-fit class` arms a man for the whole season including the night he is worst at, and the next measurement is a situational arm policy. The dokushi's failure is separate and is about **time**: the poisoned knife wins its duels (83.30% / 71.56% armoured) and still collapses the season, because the dose lengthens the fight — deaths per warrior-fight 3.9% → 4.5% and only 6.0 men reach the last night instead of 8.9. Knobs now exist for all of it: `--implement-damage`, `--implement-cycle`, `--blade-damage`, `--dose`. Details in `docs/PROGRESS.md` → 2026-09-19. **Closed 2026-09-19 — the trade is gone, at the user's call.** The implements take the katana's steel: **jitte 22/1.10, sai 22/1.15** (they were 15/1.00 and 15/1.05). What the grip costs is now **gold and a class**, not damage — the rack asks 240 and 249 for them against the katana's 160. The cycle is the katana's on purpose: at 22/1.00 an *untrained* hand with a jitte beat a katana (76.58% against 72.02%) and the implement would have been the best weapon in the game for everybody; at 1.10 the untrained hand is level (72.62%) and the class is what the grip is worth. A trained torite reads **83.40%** in the duel and **40.62%** against a two-handed enemy, against the katana's 72.02% and 23.26%, and over a season the armed dojo takes **17.5 / 19.8 / 18.2%** of last nights against a control's 19.1 / 20.2 / 19.2% — the purchase is no longer a punishment. **Still open beside it**: the dokushi's knife (its failure is time, not damage) and `CatchStaminaCost` 16, which is what made the old trade unpayable and is left alone now that the damage answer is in. |
...
| ~~**21**~~ | ~~**The dojo cannot buy a melee weapon**~~ | **Closed 2026-09-17 — the counter was built, and the first measurement on it says the premise was wrong.** The dojo can now arm a man: `Quartermaster.EquipWeapon` sells any weapon on the rack (`EquipmentCatalogue.Rack` — everything but the fists) and **the rack hangs on the man's own page**, the roster's detail column, beside his charms and under his path. That is a fourth way out, taken over the three the row opened with: the reference game sells the gladiator his weapon from the gladiator panel (`docs/REFERENCE-DOMINA-UI.md` §3, the PRIMARY slot), and arming a man is a decision about the man rather than about his kit — the armoury counter stays what it is, six regions and their wear. The price is read off the weapon's **output rate** (damage ÷ attack cycle × `WeaponGoldPerDamageRate` 8), with a share for the grip that catches and another for the dose (§11); the mastery does not travel, so changing weapon costs a veteran what he built on the old one and gives it back untouched if he buys that weapon again. **What the measurement then said:** arming the two melee classes with their own implements is a **loss at every price**. Six seeds × 1600 dojos × 180 days, paired per seed, on the documented rich bed with `--train-classes on`, the policy rearming every taught hand against a control that rearms nobody — the torite's last night falls **19.1% → 1.8%**, dojos closed **28.8 → 34.2%**, net per fight **35.7 → 20.6**; the dokushi's night **19.1% → 3.7%**, closed **28.9 → 32.4%**, net **35.6 → 21.1**. **Price is not the cause:** at `--weapon-gold 0.01` — about **9 gold** for the whole season instead of ~780 — the collapse is the same (torite 0.8 / 1.2 / 2.1%, dokushi 3.4 / 3.1 / 3.8%). So the class layer's campaign weakness was never the missing counter; it is the implements' own combat numbers, which is **#19 widened** (a torite fights better with a katana than with a jitte at equal mastery, measured 2026-09-12). This row closes and hands its remainder there: the jitte's and the sai's damage/speed/disarm share, and the dose's, are Phase 9 work. `WeaponGoldPerDamageRate` therefore stands as a **proposal, not a locked number** — a price cannot be swept for a purchase nobody should make yet |
| ~~**22**~~ | ~~**The pull-out order in a played season**~~ | **Closed 2026-09-19 — `--play` takes the order at its word.** Opened 2026-09-17 by six hand-played seasons: five of the six players reported `pull:` as broken. They were reading the behaviour correctly — the order was the core's `RetreatWhenLosing`, which fires only when the party is **both** under its health share **and** outnumbered on the field (`Combat/IRetreatPolicy.cs`), so a party of four against one or two enemies could not trip it until three of them were down, and a lone man never could. It was first closed the same day by stating the rule instead of changing it; the user reversed that on 2026-09-19. **`pull:0.5` in a played season is now `RetreatBelowHealth`: pull out when the party is under that share of health, whatever the count on the field.** The **batch bed keeps `RetreatWhenLosing`**, and for the reason it was written: an order watching health alone abandons more than 80% of 3v3 fights, and the victory rate then measures the policy rather than the balance. **Not carried into the game**: the arena's own key (`Battle.CommandRetreat`) is unconditional and immediate, and stays that way — the harness's order is a stand-in for a hand that is not there, not a copy of the key |
| ~~**23**~~ | ~~**The dokushi's knife, and the sickness proposed for it**~~ | **Closed 2026-09-21 — a move, and a little of a medicine beside it.** `PoisonBackstepSeconds` **1.0**, `PoisonBackstepHeadroom` **1.0** and `PoisonAccuracyPenaltyAtMaxDose` **0.15**; Evasion and the slow stay at **0**, measured rather than skipped (Evasion moves the duel 0.14 points, the slow measures *negative* against armour, 71.56 → 70.49%). **The move waits on the dose cap, not on a clock** — the user's correction, and the rule the row closed on: a poisoner steps out of reach only while his *own target* is carrying so much poison that the next strike's dose would be thrown away, and he goes looking for an opening the moment a fresh dose would land. The seconds are only a ceiling on the wait. Reading the nearest body instead of his own target was the bug in the first version: on a crowded field the nearest man is usually a teammate's fight, and he would walk away from a fresh man he could have poisoned. On the locked build the poisoned knife reads **87.87%** in the duel, **81.55%** against an armoured enemy — the best armoured reading of any setting measured — and **77.79%** unclassed, with the steel untouched (tantō 26.13%). In the season the armed dokushi reads **16.57%** of last nights against a 16.20% control, **+0.4**, which is the armed torite's own band (+0.6, re-measured on the same three seeds); standing still cost 2.3. The sickness fell from 0.25 to 0.15 when the rule changed, because the better move carries more of the weight: at the locked ceiling the season reads 15.77% with no sickness at all, 16.57% at 0.15, 17.13% at 0.25 — and the last of those puts the knife clear of the torite, which ends the trade. **The charge was measured first and is not the answer**: silencing the dokushi's charge (`PoisonChargeAppetite`, `--class-charge`, left at 1.0) lifts the duel to 89.55% and the whole season line by 2.7 points but lifts the *unarmed* dokushi by exactly as much, so the knife's cost stayed 2.2. It leaves a finding for a round of its own: for the dojo's own men the charge wins duels and loses seasons. **Left open beside the close, both Phase 9:** the step wants a different length by enemy (0.2 s in the open, 1.2 s in front of ō-yoroi) and today it is one number, and a poisoned *thrown* weapon does not trigger it because the gate reads the melee weapon. Details in `docs/PROGRESS.md` → 2026-09-21. **The original row:** Opened 2026-09-20, split off #19's close. The poisoned knife wins its fights (duel **83.10%**, against an armoured enemy **71.95%**, against the katana's 72.02 / 26.01) and still costs the season, because the fight it wins is **long** and a long fight is paid for in the blood of the man who wins it. **First finding: most of the old collapse was a bug, not the knife.** The rack compared weapon names and the forge renames what it reworks, so an armed dojo rebought and reforged the implement every day — about a thousand gold a season and a mastery that never settled (`docs/PROGRESS.md` → 2026-09-20). Fixed with `Weapon.SameKind`. Re-measured, 6 seeds × 1600 dojos: the armed dokushi reads **14.17%** of last nights against a control's **15.60%** — 1.4 points, not the 3.7-against-19.1 of 2026-09-17. **What the lever is not:** the dose (doubling it buys 0.6 points, the cap eats the rest) and the blade's steel (7 → 15 buys 4.7 points but ends the trade the knife is built on). **What it is:** *when* the dose is paid. The same total in half the window (tick 1.0 → 0.5 s, lifetime 6 → 3 s) reads **15.90%**, level with the sword, while the control that keeps the throughput and only splits the ticks moves nothing (14.17%). **The user's answer, and the one this row is really about: the dose is a sickness.** Nausea and a swimming head rather than more damage — three shares scaled by how far the dose has gone (`PoisonAccuracyPenaltyAtMaxDose`, `PoisonEvasionPenaltyAtMaxDose`, `PoisonSlowAtMaxDose`, `--sick-accuracy` / `--sick-evasion` / `--sick-slow`), which do not shorten the fight but make its length **harmless**: the poisoned enemy strikes worse while he dies. Measured: Accuracy 0.5 alone **15.83%** (duel 88.03%, armoured 75.93%), the three together at 0.2 / 0.3 / 0.15 **15.73%** (87.77 / 72.45), both level with the sword; Evasion alone does nothing and slowing alone is negative against armour. The sickness lifts the **unclassed** dose too (74.56 → 78.90%), so the class gap stays the thin ~8 points §4 flagged, while the 3 s window widens it to 15.0. **The three choices left to the user:** (a) the sickness alone at 0.2 / 0.3 / 0.15, (b) Accuracy 0.5 alone, (c) half the sickness plus the 3 s window — wide class gap, 1.1 points ahead of the sword, the band the armed torite sits in (+0.5). At the time of writing the three shares still defaulted to **0**. **A fourth answer, measured 2026-09-21, and it is a move rather than a medicine: hit and run.** `CombatState.Backstep` — once the dose is in, the poisoner walks out of the enemy's reach and waits there; he does not strike while he walks and the step ends when the dose does (`PoisonBackstepSeconds`, **0** by default, `--backstep`). On the duel bed it is worth 11 points in the open (83.30 → 94.98% at 0.2 s) and 14 in front of ō-yoroi (71.56 → 86.16% at 1.2 s), and the two beds want opposite lengths because armour decides whether the blade is still doing work. In the season, at 1.5 s, the armed dokushi reads **15.77%** of last nights against the nobody-armed control's 16.20% — **0.4 points, inside the bed's noise**, where standing still costs 2.3. It reaches only the man carrying the dose: with `--arm-fit none` the season is identical seed by seed with the move on and off. **The charge was measured first and is not an answer** — silencing the dokushi's charge (`PoisonChargeAppetite`, `--class-charge`) lifts the duel to 89.55% and the whole season line by 2.7 points, but lifts the unarmed dokushi by exactly as much, so the knife's cost stays 2.2. It leaves a finding for a round of its own: for the dojo's own men the charge wins duels and loses seasons. The four the call was made from: (a) the sickness at 0.2 / 0.3 / 0.15, (b) Accuracy 0.5 alone, (c) half the sickness plus the 3 s window, (d) the backstep at ~1.5 s — **and the answer was (d) with a quarter of (b)**, which is the close at the head of this row. |
| **24** | **The charge was locked on a bed that cannot see what it costs** | **Opened 2026-09-21**, out of #23's close, and it is the largest single number this project has measured. Every charge figure in §11 — the appetite curve, the windup, the speed, the arrival damage, the counter — was locked on a 20.000-fight duel bed, which asks one question (did he win) and closes the books when the fight ends. The season asks a different one: *with what left*. `PlayerChargeAppetite` / `EnemyChargeAppetite` (`--player-charge` / `--enemy-charge`, both **1.0**, nothing changed yet) were written to separate the two, because `--charge-chance` moves both sides at once and therefore measures a different fight rather than a different policy. On the documented rich bed, 3 seeds × 1600 dojos × 180 days, sweeping **only the dojo's own side**: last nights won **15.97% → 19.40 → 26.07 → 33.63 → 44.13%** at appetites 1.0 / 0.75 / 0.5 / 0.25 / 0, dojos closed **31.0 → 16.2%**, deaths per warrior-fight **4.67 → 3.03%**, net per fight **41.0 → 63.3**. Monotone, every seed, no knee. **The same duel bed sees almost none of it:** with the player's charge alone silenced, 3v3 moves −0.15 points, katana-armored −0.67, and the tantō and the poisoned knife actually *gain* (26.13 → 30.34%, 87.87 → 92.95%). **The asymmetry is the finding.** Silencing the *enemy's* charge instead is worth only +2.9 points of last night (15.97 → 18.83%), an order less than silencing the player's own; with both sides silenced the season reads **61.67%** of last nights and 1.93% deaths. So the charge is a loss for whoever throws it, and it costs the dojo far more because the dojo's men are the ones who carry the injury into the next day — the enemy is rebuilt free every fight, which is exactly the cost a duel bed cannot price. **What is not being claimed:** that the charge should go. It is a real decision with a real payoff inside one fight, chat watches it, and §11's numbers were not wrong about the fight they measured. What is claimed is that the appetite curve (0.35-1.00, read off Aggression) was calibrated without its season price, and that price is ~28 points of last night. **The round owed:** re-measure §11's five numbers on the campaign bed, decide whether the cost belongs in the curve, in the windup's exposure, or in an AI policy the player can set; and give the other classes the sweep the dokushi got. Phase 9, and it should come before any further class balance — the classes are being tuned inside a behaviour that costs more than any of them. Details in `docs/PROGRESS.md` → 2026-09-21. |
...
| ~~8~~ | ~~Honour threshold numbers~~ | **Measured and locked 2026-09-12** (400 dojos x 180 days, documented policy). **Seppuku threshold 30**: swept 20 / 30 / 40 -> dojos closed 22.0 / 26.2 / **36.5%**; above 30 the tribunal starts eating rosters a season would otherwise carry. **Honour decay 0.5 a day**: swept 0 / 0.5 / 1.5 -> closed 36.5 / 26.2 / 19.5% and dojos crossing the threshold 46.0 / 21.2 / 3.0% - at 0 a bad week never heals, at 1.5 honour stops meaning anything. **Retreat penalty 8**: it measures as nothing against a policy that rarely pulls out, so it was swept against one that flees at 60% health - 0 / 8 / 20 -> closed 43.2 / 55.2 / 67.5%, first seppuku on day 34 / 9 / 5. A bug was found doing it: the **aftermath built its own honour engine on the defaults**, so the dojo's honour numbers reached the tribunal and not the fight, and every earlier sweep of these two moved nothing. One engine now. Playtest can still move all three; what they are not any more is guesses. **Original note:**  The decay rate, the effect coefficient of targeted commands — through playtest. **The honour price of fleeing is here too** (`RetreatHonorPenalty`, §5): the rule is locked, the number is not. **The seppuku threshold was set provisionally (2026-09-04):** **30** out of 100, a pardon at **45** (§6). The user's decision — not locked, to be played with in playtest |
