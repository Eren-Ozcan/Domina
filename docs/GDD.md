...
| ~~**22**~~ | ~~**The pull-out order in a played season**~~ | **Closed 2026-09-19 — the order keeps both conditions.** Opened 2026-09-17 by six hand-played seasons: five of the six players reported `pull:` as broken. They were reading the behaviour correctly — the order is the core's `RetreatWhenLosing`, which fires only when the party is **both** under its health share **and** outnumbered on the field (`Combat/IRetreatPolicy.cs`), so a party of four against one or two enemies cannot trip it until three of them are down, and a lone man never trips it at all. **The decision is that this stays.** A player does not break off an expedition over one wound, and a policy watching health alone abandons more than 80% of 3v3 fights, which is what made the measurement bed meaningless in the first place (the reason `RetreatWhenLosing` exists). What was wrong was never the rule but that nothing said it: the order is stated once, in the played-season briefing and in `docs/played-seasons/README.md`, as "pull out when we are **losing** — outnumbered **and** under the share", so a script that sends four men at one enemy is written knowing the order cannot fire. **Not carried into the game**: the arena's own key (`Battle.CommandRetreat`) is unconditional and immediate, and stays that way — the harness's order is a stand-in for a hand that is not there, not a copy of the key |
